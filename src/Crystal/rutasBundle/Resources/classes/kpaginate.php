 ...
 *      class.kpaginate.php
 *
 *      Copyright 2009 Ismael Rodríguez <ismael.rodriguez@premper.com>
 *
		...
		public function paginate(){
			...
			if($this->num_pgs <= $this->show){
				...
				for($i = 1 ; $i <= $this->num_pgs ; $i++)
					echo '<td>'. ($i == 1 ? '' : '<span class="slash"></span>') .'<a ' . $this->createPageLink($i) . ' class="normal">' . ($i) . '</a></td>' ;
			}
			else{
				...
				for($i = $this->start ; $i <= $this->stop ; $i++)
					echo '<td><span class="slash"></span><a ' . $this->createPageLink($i) . ' class="normal">' . ($i) . '</a></td>' ;
